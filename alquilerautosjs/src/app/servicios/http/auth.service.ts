...
export class AuthService {
  ...
  constructor(private ngZone: NgZone, public afAuth: AngularFireAuth, private _db: AngularFirestore, private router: Router) {
  }
  ...
  async login(email: string, password: string): Promise<User> {
    try {
      ...
      this._db.collection('usuario').ref.where('email', '==', user.email).onSnapshot(snap => {
  ...
  async register(usuarioData): Promise<void> {
    try {
      const {nombre, apellido, email, password, cedula, telefono} = usuarioData;
  ...
  async userChanges(): Promise<void> {
    await this.afAuth.onAuthStateChanged(async currentUser => {
      if (currentUser) {
        await this._db.collection('usuario').ref.where('email', '==', currentUser.email).onSnapshot(snap => {
          snap.forEach(userRef => {
            this.currentUser = userRef.data();
            this.setUserStatus(this.currentUser);
            if (userRef.data().rol === 'administrador' && window.location.href.includes('administrador')) {
              this.ngZone.run(() => this.router.navigate(['/administrador']));
            } else if (userRef.data().rol === 'cliente' && window.location.href.includes('cliente')) {
